    ...
    function submit()
    {
        ...
        {         
			$swap = array(
							...
							'delete_link'		=> $PREFS->item('cp_url').'?S=0&D=cp&C=addons_modules&M=show_module_cp&module=comment'.'&method=delete_comment_confirm'.'&channel_id='.$channel_entry->row('channel_id').'&entry_id='.$_POST['entry_id'].'&comment_id='.$comment_id,
                            'approve_link'		=> $PREFS->item('cp_url').'?S=0&D=cp&C=addons_modules&M=show_module_cp&module=comment'.'&method=change_comment_status'.'&channel_id='.$channel_entry->row('channel_id').'&entry_id='.$_POST['entry_id'].'&comment_id='.$comment_id.'&status=o',
                            'close_link'		=> $PREFS->item('cp_url').'?S=0&D=cp&C=addons_modules&M=show_module_cp&module=comment'.'&method=change_comment_status'.'&channel_id='.$channel_entry->row('channel_id').'&entry_id='.$_POST['entry_id'].'&comment_id='.$comment_id.'&status=c'
						 );
