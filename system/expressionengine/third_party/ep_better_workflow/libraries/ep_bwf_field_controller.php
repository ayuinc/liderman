				...
				if(is_object($ft))
				{
					...
					foreach($this->fields_to_process as $the_field)
					{
						...
						if(in_array($field_type, $the_field[0]))
						{
							...
							$this->action_logger->add_to_log("ep_field_controller: process_draft_data(): Pre processed value for field type " . $field_type . ": " . $revision_post_data[$key]);
							...
							$this->action_logger->add_to_log("ep_field_controller: process_draft_data(): Response from internal field method for field type " . $field_type . ": " . $revision_post_data[$key]);
						}
					}
					...
					if (!$process_internally) {
						...
						switch ($draft_action)
						{
							...
							//else
							//{
							//	$this->action_logger->add_to_log("Native Save() for ".$field_type);
							//	$this->action_logger->add_to_log("Native method_exists(post_save) for ".$field_type . ' -- ' . method_exists($ft, 'post_save'));
							//	$this->action_logger->add_to_log("Native method_exists(save) for ".$field_type . ' -- ' . method_exists($ft, 'save'));
							//	
							...
							//		
							//		$this->action_logger->add_to_log("Native Save(): " . $revision_post_data[$key]);
							//	}
							//}	
							break;
